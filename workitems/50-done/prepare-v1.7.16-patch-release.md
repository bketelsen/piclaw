...
- [x] Delta review is written down with committed vs uncommitted scope called out explicitly.
- [x] A patch-release notes draft exists for review before tagging.
- [x] Relevant supporting workitems are updated or moved so follow-up work is not left implicit.
- [x] Candidate release scope is reduced to a clean working tree.
- [x] Full release validation passes locally (`make build-piclaw`, `make test`, `make ci-fast`).
- [x] Version bump, tag, push, and GitHub release are ready to execute without guesswork.
...
- [x] Decide whether the current dirty working tree belongs in `v1.7.16` or should be split out.
- [x] Confirm rebuilt generated/dist artifacts are in sync with the final source scope.
- [x] Confirm whether any manual browser smoke is needed for code-copy buttons and timing surfaces.
...
- [x] Narrow browser smoke recommended for code-copy buttons and hot-path timing visibility if the final patch keeps the web tranche together.

## Definition of Done

- [x] Release scope is explicitly decided and clean
- [x] Release notes draft is ready for human review
- [x] Supporting tickets/docs are updated to match scope
- [x] Local release gate is green
- [x] Ticket front matter and updates reflect the actual outcome
...
### 2026-04-17
- Closed the ticket and moved it to `50-done` after confirming the `v1.7.16` patch-release prep work is complete.
